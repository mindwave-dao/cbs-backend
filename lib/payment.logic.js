...
export async function createInvoiceLogic(req, res) {
    ...
    try {
        ...
        // 🟢 METADATA ONLY (Name/Email in metadata, no anonymous flag)
        // 🟢 AUTHORITATIVE 3THIX FLOW
        // Endpoint: /order/payment/create
        // Payload: Must include 'cart' and 'rail'
        const targetUrl = `${THIX_API_URL}/order/payment/create`;
        ...
        const payload = {
            ...
            cart: [
                {
                    product_name: "NILA Token",
                    qty_unit: quantity || 1,
                    price_unit: Number(amount).toFixed(2)
                }
            ],
        ...
        if (!response.ok) {
            ...
            return res.status(502).json({
                success: false,
                error: "3thix order creation failed",
        ...
        try {
            ...
            return res.status(502).json({ success: false, error: "Invalid JSON from 3thix", raw: text });
        }
        ...
        // 🟢 EXTRACT AUTHORITATIVE ID
        const thixInvoiceId = data.invoice_id || data.order_id; // Prefer invoice_id
        if (!thixInvoiceId) {
            console.error("[3THIX ERROR] No invoice_id returned", data);
            return res.status(502).json({ success: false, error: "No invoice_id in response", raw: data });
        }

        // 🟢 CONSTRUCT REDIRECT URL (CHECKOUT)
        // Format: https://pay.3thix.com/checkout?invoiceId={THIX_ID}&callbackUrl={...}
        const paymentUrl = `${PAYMENT_PAGE_BASE}/checkout?invoiceId=${thixInvoiceId}&callbackUrl=${encodeURIComponent(callbackUrl)}`;

        console.log(`[CREATE_INVOICE] Redirecting to: ${paymentUrl}`);
