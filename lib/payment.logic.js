...
export async function createInvoiceLogic(req, res) {
    ...
    try {
        ...
        // [NEW] Persist Intent ID mapping immediately for robustness
        // This ensures verify-intent can find the invoiceId even if frontend loses it
        try {
            ...
            console.log(`[CREATE_INTENT] Saved PENDING intent mapping: ${intentId} -> ${invoiceId}`);
        } catch (err) {
            console.error(`[CREATE_INTENT] Failed to save intent mapping: ${err.message}`);
            // Non-blocking error, proceed to return URL
        }
