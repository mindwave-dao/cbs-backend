...
/* ---------- Logic: Check/Process Status (Webhook/Finalization) ---------- */
export async function handlePaymentLogic(invoiceId, sourceLabel = '3THIX_API', importData = null) {
    ...
    if (incomingStatus === 'SUCCESS') {
        ...
        // Update Sheet: STATUS=SUCCESS, EmailSent=NO (handled by updateTransactionStatus)
...
function normalizeStatus(s) {
    if (!s) return 'PENDING';
    s = s.toUpperCase();
    if (['PAID', 'COMPLETED', 'SUCCESS', 'INVOICE_PAID', 'ORDER_COMPLETED', 'APPROVED'].includes(s)) return 'SUCCESS';
