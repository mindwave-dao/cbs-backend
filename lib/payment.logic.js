...
export async function createInvoiceLogic(req, res) {
    const {
        ...
        email,
        billing_data,
        // Integration type: "iframe" (default) or "url"
    ...
    try {
        ...
        console.log(`[CREATE_INTENT] [STEP 2] Saved pending transaction: ${invoiceId}`);
    } catch (dbError) {
    ...
    try {
        ...
        console.log(`[CREATE_INTENT] [STEP 3] Calling 3thix create-intent API: ${targetUrl}`);
        ...
        const payload = {
            ...
            email: email,
            name: name,
            // Order items (required for server to calculate totals)
            order_items: [
                {
            ...
            // Consolidated order object (preferred format)
            order: {
                order_id: invoiceId,
                order_currency: currency.toUpperCase(),
                order_total: Number(amount) * (Number(quantity) || 1),
                items: [
                    {
                        name: "NILA Token Purchase",
                        sku: "NILA-TOKEN",
                        quantity: Number(quantity) || 1,
                        price: Number(amount),
                        subtotal: Number(amount) * (Number(quantity) || 1),
                        total: Number(amount) * (Number(quantity) || 1)
                    }
                ],
                billing: billingInfo,
                order_summary: {
                    order_subtotal: Number(amount) * (Number(quantity) || 1),
            ...
            billing: billingInfo,
            // Custom metadata (for our reference when webhook fires)
            metadata: {
        ...
        if (integration_type === "url") {
            payload.success_url = success_url || `${FRONTEND_BASE_URL}/payment-success?invoiceId=${invoiceId}`;
            payload.cancel_url = cancel_url || `${FRONTEND_BASE_URL}/payment-cancelled?invoiceId=${invoiceId}`;
            if (fail_url) {
        ...
        console.log(`[CREATE_INTENT] [${correlationId}] 3thix Parsed Response:`, JSON.stringify(data));
        ...
        console.log(`[CREATE_INTENT] [SUCCESS] Intent created for ${invoiceId}, Payment URL: ${paymentUrl}`);
        return res.json({
            success: true,
            invoiceId,
            intentId,
