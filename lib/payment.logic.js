...
export async function createInvoiceLogic(req, res) {
    ...
    try {
        ...
        await syncToPaymentTransactions(null, invoiceId, "CREATED", email, name, "", "", walletAddress); // passing null for sheets client to let it init internally if supported, or I need to pass it?
        ...
        if (sheets) {
            await syncToPaymentTransactions(sheets, invoiceId, "CREATED", email, name, "", "", walletAddress);
        }
