...
export async function createInvoiceLogic(req, res) {
    ...
    try {
        ...
        // Background patch - don't block response too long, but good to wait to ensure consistency
        ...
        if (paymentUrl) {
            ...
            // Fallback (Legacy)
            redirectUrl = `${PAYMENT_PAGE_BASE}?invoiceId=${invoiceId}&returnUrl=${encodeURIComponent(finalReturnUrl)}`;
        }
