import { getSheets } from "./googleClient.js";
...
// NO CHANGE TO appendToActivityLog
...
export async function updateTransactionStatus(invoiceId, status, extraData = {}) {
    ...
    if (status === 'SUCCESS') {
        ...
        if (tokenPrice) requests.push({ range: `PAYMENT_TRANSACTIONS!I${rowIndex}`, values: [[tokenPrice.toString()]] });
        if (tokens) requests.push({ range: `PAYMENT_TRANSACTIONS!J${rowIndex}`, values: [[tokens.toString()]] });
