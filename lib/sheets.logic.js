...
const TRANSACTIONS_HEADERS = [
    ...
    "TOKEN_PRICE", "TOKENS_PURCHASED", "ADMIN_EMAIL_SENT", "WALLET_ADDRESS"
];

const ADDITIONAL_INFO_HEADERS = [
    "merchant_ref_id", "invoice_id", "name", "email", "timestamp"
];
...
export async function updateTransactionStatus(invoiceId, status, extraData = {}) {
    ...
    // Wallet Address (Col J)
    if (!tx.WALLET_ADDRESS && extraData.walletAddress) {
        requests.push({ range: `PAYMENT_TRANSACTIONS!J${rowIndex}`, values: [[extraData.walletAddress]] });
    }
