...
// Crypto Payment Headers (ISOLATED from Card Flow)
// Crypto Payment Headers (Strict Order, Human Readable)
const CRYPTO_PAYMENTS_HEADERS = [
    "Timestamp",
    "Full Name",
    "Email",
    "Wallet Address",
    "Amount",
    "Estimated NILA Tokens",
    "Network",
    "Tx Hash (Last 6)",
    "Status"
];

// Audit Log Headers (Immutable)
...
// Email Logs Headers (Brevo Webhooks)
...
export async function updateTransactionStatus(invoiceId, status, extraData = {}) {
    ...
    if (status === 'SUCCESS') {
        ...
        const updates = [
            ...
            { range: `PAYMENT_TRANSACTIONS!G${rowIndex}`, values: [[(amount || 0).toString()]] },
            { range: `PAYMENT_TRANSACTIONS!H${rowIndex}`, values: [[currency || ""]] },
            { range: `PAYMENT_TRANSACTIONS!I${rowIndex}`, values: [[(tokenPrice || 0).toString()]] },
            { range: `PAYMENT_TRANSACTIONS!J${rowIndex}`, values: [[(tokens || 0).toString()]] },
            { range: `PAYMENT_TRANSACTIONS!N${rowIndex}`, values: [[now]] }
        ];

        console.log(`[SHEETS] Attempting atomic update for ${invoiceId} (Row ${rowIndex}) with data:`, {
            status, email, amount, tokens
        });
        ...
        try {
            ...
            console.error(`[SHEETS] Failed atomic update for ${invoiceId}: ${e.message}`, e);
        }
...
/**
 * Appends crypto payment submission to CRYPTO_PAYMENTS sheet
 * COMPLETELY ISOLATED from PAYMENT_TRANSACTIONS
 * 
 * @param {object} row - Array of values matching CRYPTO_PAYMENTS_HEADERS
 */
export async function appendToCryptoPayments(row) {
    const client = getSheetsClient();
    const sheetId = process.env.GOOGLE_SHEET_ID;
    if (!client || !sheetId) return;

    await ensureHeaders("CRYPTO_PAYMENTS", CRYPTO_PAYMENTS_HEADERS);

    try {
        await client.spreadsheets.values.append({
            spreadsheetId: sheetId,
            range: "CRYPTO_PAYMENTS!A:I",
            valueInputOption: "RAW",
            insertDataOption: "INSERT_ROWS",
            requestBody: { values: [row] }
        });
        console.log(`[SHEETS] Crypto payment logged for ${row[2]}`); // Email is index 2
    } catch (e) {
        console.error(`[SHEETS] Failed to append to CRYPTO_PAYMENTS: ${e.message}`);
    }
}

/**
 * Appends to AUDIT_LOGS sheet
 * @param {Array} row - [timestamp, admin, role, action, resource, refId, ip]
 */
export async function appendToAuditLog(row) {
    const client = getSheetsClient();
    const sheetId = process.env.GOOGLE_SHEET_ID;
    if (!client || !sheetId) return;

    await ensureHeaders("AUDIT_LOGS", AUDIT_LOG_HEADERS);

    try {
        await client.spreadsheets.values.append({
            spreadsheetId: sheetId,
            range: "AUDIT_LOGS!A:G",
            valueInputOption: "RAW",
            insertDataOption: "INSERT_ROWS",
            requestBody: { values: [row] }
        });
        console.log(`[AUDIT] Logged action: ${row[3]}`);
    } catch (e) {
        console.error(`[SHEETS] Failed to append to Audit Log: ${e.message}`);
    }
}

/**
 * Appends to EMAIL_LOGS sheet
 * @param {Array} row - [timestamp, messageId, email, subject, event, status, reason, ip, userAgent]
 */
export async function appendToEmailLogs(row) {
    const client = getSheetsClient();
    const sheetId = process.env.GOOGLE_SHEET_ID;
    if (!client || !sheetId) return;

    await ensureHeaders("EMAIL_LOGS", EMAIL_LOGS_HEADERS);

    try {
        await client.spreadsheets.values.append({
            spreadsheetId: sheetId,
            range: "EMAIL_LOGS!A:I",
            valueInputOption: "RAW",
            insertDataOption: "INSERT_ROWS",
            requestBody: { values: [row] }
        });
        console.log(`[EMAIL_LOGS] Logged event: ${row[4]} for ${row[2]}`);
    } catch (e) {
        console.error(`[SHEETS] Failed to append to Email Logs: ${e.message}`);
    }
}

/**
 * Fetches ALL transactions from Payment and Crypto sheets
 * Returns normalized, sorted array (Newest first)
 */
export async function getAllTransactions({ type, status, dateFrom, dateTo }) {
    const client = getSheetsClient();
    const sheetId = process.env.GOOGLE_SHEET_ID;
    if (!client || !sheetId) return { data: [] };
    ...
    if (!type || type === 'crypto' || type === 'all') {
        try {
            ...
            for (let i = 1; i < rows.length; i++) {
                ...
                all.push({
                    source: "CRYPTO",
                    referenceId: r[CRYPTO_SCHEMA.TX_HASH_LAST6] || `row-${i}`,
                    timestamp: r[CRYPTO_SCHEMA.TIMESTAMP],
                    customerName: r[CRYPTO_SCHEMA.FULL_NAME],
                    email: r[CRYPTO_SCHEMA.EMAIL],
                    amount: parseFloat(r[CRYPTO_SCHEMA.AMOUNT] || 0),
                    currency: "USD", // Crypto implied USD value?
                    network: r[CRYPTO_SCHEMA.NETWORK],
                    txHashLast6: r[CRYPTO_SCHEMA.TX_HASH_LAST6],
                    status: r[CRYPTO_SCHEMA.STATUS],
                    rowIndex: i + 1 // Useful for updates
                });
            }
        } catch (e) { console.error("Error fetching crypto:", e.message); }
    }

    // 2. Fetch Card (PAYMENT_TRANSACTIONS)
    if (!type || type === 'card' || type === 'payment' || type === 'all') {
        try {
            const res = await client.spreadsheets.values.get({
                spreadsheetId: sheetId,
                range: "PAYMENT_TRANSACTIONS!A:N"
            ...
            for (let i = 1; i < rows.length; i++) {
                const r = rows[i];
                // Mapping using SHEET_SCHEMA
                all.push({
                    source: "CARD",
                    referenceId: r[SHEET_SCHEMA.INVOICE_ID],
                    timestamp: r[SHEET_SCHEMA.CREATED_AT],
                    customerName: r[SHEET_SCHEMA.NAME],
                    email: r[SHEET_SCHEMA.EMAIL],
                    amount: parseFloat(r[SHEET_SCHEMA.AMOUNT] || 0),
                    currency: r[SHEET_SCHEMA.CURRENCY] || "USD",
                    network: r[SHEET_SCHEMA.WALLET_NETWORK] || "N/A",
                    txHashLast6: null,
                    status: r[SHEET_SCHEMA.STATUS],
                    rowIndex: i + 1
                });
            }
        } catch (e) { console.error("Error fetching card transactions:", e.message); }
    }

    // 3. Filter
    if (status) {
        all = all.filter(t => t.status?.toLowerCase() === status.toLowerCase());
    }
    if (dateFrom) {
        const from = new Date(dateFrom).getTime();
        all = all.filter(t => new Date(t.timestamp).getTime() >= from);
    }
    if (dateTo) {
        const to = new Date(dateTo).getTime();
        all = all.filter(t => new Date(t.timestamp).getTime() <= to);
    }
...
/**
 * Fetches EMAIL_LOGS from the sheet
 * Returns normalized array (Newest first)
 */
export async function getEmailLogs({ email, limit = 50 }) {
    const client = getSheetsClient();
    const sheetId = process.env.GOOGLE_SHEET_ID;
    if (!client || !sheetId) return { logs: [] };
    ...
    try {
        ...
        // Map to object [Timestamp, MessageID, Email, Subject, Event, Status, Reason, IP, UserAgent]
        let logs = rows.map(r => ({
            timestamp: r[0],
            messageId: r[1],
            email: r[2],
        ...
        console.error(`[SHEETS] getEmailLogs failed: ${e.message}`);
        return { logs: [], error: e.message };
    }
}

/**
 * Schema for CRYPTO_PAYMENTS columns (0-indexed)
 */
export const CRYPTO_SCHEMA = {
    TIMESTAMP: 0,
    FULL_NAME: 1,
    EMAIL: 2,
    WALLET_ADDRESS: 3,
...
/**
 * Finds a crypto payment by txHashLast6 and email
 * Returns { rowIndex, data } or null
 */
export async function findCryptoPayment(txHashLast6, email) {
    const client = getSheetsClient();
    const sheetId = process.env.GOOGLE_SHEET_ID;
    if (!client || !sheetId) return null;
    ...
    try {
        ...
        for (let i = 1; i < rows.length; i++) {
            const row = rows[i];
            // Match by txHashLast6 (col 7) AND email (col 2) - case insensitive
            if (
                row[CRYPTO_SCHEMA.TX_HASH_LAST6]?.toLowerCase() === txHashLast6?.toLowerCase() &&
                row[CRYPTO_SCHEMA.EMAIL]?.toLowerCase() === email?.toLowerCase()
            ) {
                return {
                    rowIndex: i + 1, // 1-based for Sheets API
                    timestamp: row[CRYPTO_SCHEMA.TIMESTAMP],
                    fullName: row[CRYPTO_SCHEMA.FULL_NAME],
                    email: row[CRYPTO_SCHEMA.EMAIL],
                    walletAddress: row[CRYPTO_SCHEMA.WALLET_ADDRESS],
                    amount: row[CRYPTO_SCHEMA.AMOUNT],
                    estimatedTokens: row[CRYPTO_SCHEMA.ESTIMATED_TOKENS],
                    network: row[CRYPTO_SCHEMA.NETWORK],
                    txHashLast6: row[CRYPTO_SCHEMA.TX_HASH_LAST6],
                    status: row[CRYPTO_SCHEMA.STATUS]
                };
            }
        }
    } catch (e) {
        console.error(`[SHEETS] findCryptoPayment failed: ${e.message}`);
    }
    return null;
}

/**
 * Updates crypto payment status and returns updated row data
 * @param {number} rowIndex - 1-based row index
 * @param {string} newStatus - New status (e.g., "CONFIRMED")
 */
export async function updateCryptoPaymentStatus(rowIndex, newStatus) {
    const client = getSheetsClient();
    const sheetId = process.env.GOOGLE_SHEET_ID;
    if (!client || !sheetId) return false;

    try {
        // Status is column I (index 8, which is column 9 = I)
        await client.spreadsheets.values.update({
            spreadsheetId: sheetId,
            range: `CRYPTO_PAYMENTS!I${rowIndex}`,
            valueInputOption: "RAW",
            requestBody: { values: [[newStatus]] }
        });
        console.log(`[SHEETS] Crypto payment row ${rowIndex} updated to ${newStatus}`);
        return true;
    } catch (e) {
        console.error(`[SHEETS] updateCryptoPaymentStatus failed: ${e.message}`);
        return false;
    }
}
