...
export async function sendUserPaymentSuccessEmail(to, name, invoiceId, tokens, tokenPrice, amount, walletAddress) {
    ...
    // Note: This relies on sheets having been updated prior to this call, 
    // or this function is called in a flow where we want to double check.
 ...
 */
export async function sendCryptoUserAcknowledgementEmail(to, name, amount, estimatedTokens, network, txHash) {
    if (!to || !process.env.BREVO_API_KEY) return { success: false };

    const templateId = parseInt(process.env.BREVO_CRYPTO_USER_CONFIRMATION_TEMPLATE_ID);
    if (!templateId) {
        console.error("Missing BREVO_CRYPTO_USER_CONFIRMATION_TEMPLATE_ID");
    ...
    try {
        const res = await fetch(BREVO_API_URL, {
            ...
            body: JSON.stringify({
                ...
                params: {
                    full_name: name,
                    wallet_address: "TRON_WALLET_ADDRESS...", // Wait, checking params signature
                    // Params: to, name, amount, estimatedTokens, network, txHash
                    amount: amount,
