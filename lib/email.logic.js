...
// import fetch from "node-fetch"; // Use global fetch for Vercel compatibility
...
async function fetchWithRetry(url, options, retries = 3, backoff = 1000) {
    for (let i = 0; i < retries; i++) {
        try {
            ...
            if (!res.ok) {
                // If it's a 5xx error, we might want to retry. If 4xx, probably not (unless rate limit).
                // For simplicity, we'll return the response if it's not ok, but let the caller handle it.
                // UNLESS it's a 502/503/504 which are retryable.
            ...
            console.warn(`[FETCH FAIL] Attempt ${i + 1}/${retries} failed: ${err.message}`);
 ...
 * Uses Brevo template for consistency.
 */
export async function sendUserPaymentSuccessEmail(to, name, invoiceId, tokens, tokenPrice, amount, walletAddress, force = false) {
    ...
    if (!force && tx && tx.email_sent_user === 'YES') {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            throw new Error(`BREVO_EMAIL_FAILED: ${res.status} ${txt}`);
        }
        ...
        console.error(`[EMAIL ERROR] User Email failed for ${invoiceId}: ${e.message}`);
 ...
 * Uses Brevo template for consistency.
 */
export async function sendAdminPaymentNotification(params, force = false) {
    const {
    ...
    const tx = await findTransaction(invoiceId);
    if (!force && tx && tx.email_sent_admin === 'YES') {
        return true;
    }
...
export async function sendCryptoUserConfirmationEmail(params) {
    ...
    try {
        ...
        console.log(`[BREVO CONFIRM USER] Calling fetch...`);
        const res = await fetch(BREVO_API_URL, {
            method: "POST",
            headers: {
                "accept": "application/json",
...
export async function sendCryptoAdminConfirmationEmail(params) {
    ...
    try {
        ...
        console.log(`[BREVO CONFIRM ADMIN] Calling fetch...`);
        const res = await fetch(BREVO_API_URL, {
            method: "POST",
            headers: {
                "accept": "application/json",
