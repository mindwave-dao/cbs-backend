...
export async function sendAdminPaymentNotification(params) {
  ...
  const adminEmail = ADMIN_EMAIL || "support@mindwavedao.com";
...
async function markEmailSentInSheets(invoiceId) {
  // UPDATED: Use shared logic
  const { markEmailSent } = await import("./sheets.logic.js");
...
async function checkEmailSentStatus(invoiceId) {
  // UPDATED: Use shared logic
  const { findTransaction } = await import("./sheets.logic.js");
  const tx = await findTransaction(invoiceId);
  if (!tx) return null;
  return tx.email_sent_user === "YES" ? "YES" : null;
}
