 ...
 */
export async function sendPaymentSuccessEmail(to, name, invoiceId, tokens, tokenPrice, amount) {
  if (!to || !BREVO_API_KEY) return false;

  const senderEmail = EMAIL_FROM || "payments@mindwavedao.com";
  const senderName = "Mindwave Payments";
  // Strict separation: User email does NOT use the Admin Template ID.
...
export async function sendAdminPaymentNotification(params) {
  const {
    ...
    email, source, timestamp // 'email' here is the User Email for reference
  } = params;

  // STRICT ENV VAR: BREVO_ADMIN_TEMPLATE_ID
  const adminEmail = process.env.ADMIN_EMAIL || "support@mindwavedao.com";

  // Handle numeric or string IDs (e.g. standard int '15' or alphanumeric '5e98...')
  // parseInt is dangerous for '5e...' strings as it truncates to '5'.
  const rawId = process.env.BREVO_ADMIN_TEMPLATE_ID;
  const templateId = /^\d+$/.test(rawId) ? parseInt(rawId) : rawId;

  if (!BREVO_API_KEY || !templateId) {
    console.warn(`[ADMIN EMAIL SKIP] Missing BREVO_API_KEY or BREVO_ADMIN_TEMPLATE_ID (Value: ${rawId})`);
    return false;
  }
  ...
  try {
    ...
    const payload = {
      ...
      params: {
        ...
        CURRENCY: String(currency).toUpperCase(), // e.g. USD
        TOKENS: String(tokens), // NILA
 ...
 */
export async function processSuccessfulPayment(invoiceId, userEmail, userName, tokens, tokenPrice, amount) {
  ...
  try {
    ...
    await sendPaymentSuccessEmail(userEmail, userName, invoiceId, tokens, tokenPrice, amount);
