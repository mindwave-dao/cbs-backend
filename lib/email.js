  ...
  try {
    ...
    const response = await fetch(BREVO_API_URL, {
      ...
      body: JSON.stringify({
        ...
        subject: "NILA Tokens Purchased Successfully",
        htmlContent: `
          <!DOCTYPE html>
          <html>
          <head>
  ...
  try {
    const response = await fetch(BREVO_API_URL, {
      ...
      body: JSON.stringify({
        ...
        subject: "NILA Token Purchase Notification",
        htmlContent: `
          <!DOCTYPE html>
          <html>
...
export async function handlePostSuccessActions({
  ...
  try {
    ...
    if (userEmailSent || adminEmailSent) {
      ...
      console.log(`✅ Emails sent and marked in sheets for ${invoiceId}: user=${userEmailSent}, admin=${adminEmailSent}`);
      return true;
    } else {
    ...
    console.error("Error in post-success actions:", error.message);
    return false;
  }
}
...
async function markEmailSentInSheets(invoiceId) {
  ...
  try {
    ...
    await sheetsClient.spreadsheets.values.update({
      spreadsheetId: process.env.GOOGLE_SHEET_ID,
      range: `Transactions!E${rowIndex}:F${rowIndex}`,
