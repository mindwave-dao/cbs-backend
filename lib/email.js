...
export async function sendPaymentSuccessEmail(to, name, invoiceId) {
  ...
  try {
    ...
    // CHECK FOR 201 AND messageId
    if (!response.ok || !result.messageId) {
      console.error('[BREVO ERROR]', response.status, result);
      throw new Error('BREVO_EMAIL_FAILED');
    }

    console.log(`[BREVO SENT] messageId=${result.messageId}`);
    return true;

  } catch (error) {
    console.error("Brevo customer email error:", error.message);
    throw error; // Propagate error so processSuccessfulPayment handles it
  }
}
...
export async function processSuccessfulPayment(invoiceId, userEmail, userName) {
  ...
  try {
    ...
    await sendPaymentSuccessEmail(userEmail, userName, invoiceId);
    ...
    // Send admin email as best-effort (don't block success if this fails)
    sendAdminEmail(userName, userEmail, invoiceId).catch(err => {
      console.error("❌ Failed to send admin email (non-critical):", err.message);
    });
