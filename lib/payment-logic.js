...
/* ---------- 3Thix API Logic ---------- */
/**
 * Authoritative check against 3thix
 * 
 ...
 * The database status (updated by webhook) should be trusted as primary.
 */
export async function check3ThixAuthoritative(invoiceId) {
    ...
    if (!THIX_PUBLIC_KEY || !THIX_SECRET_KEY || !invoiceId) {
        console.warn(`[3THIX] Cannot perform authoritative check - missing keys or invoiceId`);
        return null;
    }

    try {
        console.log(`🔍 3Thix Authoritative Check (fallback): ${invoiceId}`);

        // Attempt to check intent status - endpoint may vary based on 3thix's new API
        const response = await fetchWithRetry(`${THIX_API_URL}/api/card/intent-status`, {
            method: "POST",
        ...
        if (response.ok) {
            ...
            console.warn(`[3THIX] Status check returned ${response.status}: ${errorText.substring(0, 200)}`);
            // Return null to indicate we should rely on DB status
            return null;
        }
    } catch (e) {
        console.error("3Thix Check Error:", e.message);
    }
    return null;
}
