...
export async function checkPaymentStatusLogic(invoiceId, injectedSheets = null) {
    ...
    let finalStatus = ptRow.STATUS;
    // Normalize to CREATED | PROCESSING | SUCCESS | FAILED
    // If it is AWAITING_PAYMENT, map to CREATED or PROCESSING? User said: "CREATED | AWAITING_PAYMENT | SUCCESS | FAILED"
    // We stick to what is in the sheet.

    // Safety: DO NOT WRITE PROCESSING.
    ...
    if (finalStatus === "SUCCESS") {
        return {
            ...
            walletAddress: ptRow.WALLET_ADDRESS,
            walletNetwork: ptRow.WALLET_NETWORK
        };
    }

    // Default for Created / Failed / Other
    return {
        status: finalStatus, // e.g., CREATED, FAILED
        invoiceId: ptRow.INVOICE_ID,
        amount: ptRow.AMOUNT,
