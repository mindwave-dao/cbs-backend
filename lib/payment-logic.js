...
export async function findInPaymentTransactions(sheets, invoiceId) {
    try {
        const response = await sheets.spreadsheets.values.get({
            spreadsheetId: GOOGLE_SHEET_ID,
            range: "PAYMENT_TRANSACTIONS!A2:J" // Expanded range to J (index 9)
        });
        const rows = response.data.values || [];
...
export async function syncToPaymentTransactions(sheets, invoiceId, status, email, name, tokenPrice = '', tokensPurchased = '', walletAddress = '') {
    if (!sheets) return;
    try {
        const existing = await findInPaymentTransactions(sheets, invoiceId);
        if (!existing) {
            // Append with new columns, initializing ADMIN_EMAIL_SENT explicitly as 'NO' and WALLET_ADDRESS
            await sheets.spreadsheets.values.append({
                spreadsheetId: GOOGLE_SHEET_ID,
                range: "PAYMENT_TRANSACTIONS!A2:J",
                valueInputOption: "RAW",
                insertDataOption: "INSERT_ROWS",
                requestBody: { values: [[invoiceId, status, email || '', name || '', 'NO', '', tokenPrice, tokensPurchased, 'NO', walletAddress]] }
            });
        }
...
/* ---------- Read-Only Status Check Logic ---------- */

async function findInTransactions(sheets, invoiceId) {
    try {
        const response = await sheets.spreadsheets.values.get({
            spreadsheetId: GOOGLE_SHEET_ID,
            range: "Transactions!A2:N" // Covers up to column N (index 13)
        });
        const rows = response.data.values || [];
        // invoice_id is at index 6
        const row = rows.find(r => r[6] === invoiceId);
        if (row) {
            return {
                amount: row[2],
                currency: row[3],
        ...
        console.warn("[Transactions Check Failed]", e.message);
    }
    return null;
}
...
export async function checkPaymentStatusLogic(invoiceId, injectedSheets = null) {
    ...
    // 1. Read from sheets
    const [txRow, ptRow] = await Promise.all([
        findInTransactions(sheets, invoiceId),
        findInPaymentTransactions(sheets, invoiceId)
    ]);
    ...
    let status = "PROCESSING"; // Default if found but status not explicitly SUCCESS/FAILED

    if (ptRow) {
        if (ptRow.STATUS === "SUCCESS") status = "SUCCESS";
        else if (ptRow.STATUS === "FAILED") status = "FAILED";
        else if (ptRow.STATUS === "PROCESSING") status = "PROCESSING";
        // If ptRow exists but status is something else (e.g. PENDING), it defaults to PROCESSING as per "only return PROCESSING if... exists"
        // actually prompt says: "ONLY return PROCESSING if the invoice EXISTS and status column explicitly says PROCESSING."
        // But what if it says PENDING?
        // "Ignore frontend assumptions."
        // "If PAYMENT_TRANSACTIONS.STATUS === 'PROCESSING' -> Payment is NOT finalized"
        // Let's stick to: if it exists, use its status, or map to PROCESSING if pending?
        // User said: "ONLY return PROCESSING if the invoice EXISTS and status column explicitly says PROCESSING."
        // This implies if it says "PENDING", maybe we should return "PENDING"?
        // But the allowed response contract only showed SUCCESS, PROCESSING, FAILED, NOT_FOUND.
        // Let's look at `normalize3ThixStatus` in the same file. It normalizes to SUCCESS, FAILED, PARTIAL, PROCESSING.
        // If ptRow.STATUS is 'PENDING', let's treat as PROCESSING for now to be safe, or just pass it through?
        // Case 2 says: "Invoice FOUND – PROCESSING".
        // Let's assume defaults for non-final states are PROCESSING.
    } else if (txRow) {
        // If only in Transactions (legacy or partial), assume PROCESSING?
        // User said: "Sheet lookup is the ONLY source of truth"
        // If it's in Transactions but not PaymentTransactions, it might be an old record?
        // "Read from TWO sheets".
        status = "PROCESSING";
    }

    // 3. Data Extraction
    const amount = txRow ? txRow.amount : null;
    const currency = txRow ? txRow.currency : null;
    const timestamp = txRow ? txRow.timestamp : null;

    let tokenPrice = null;
    let tokensPurchased = null;
    let walletAddress = null;

    if (status === "SUCCESS" && ptRow) {
        // Parse numbers safely
        const price = parseFloat(ptRow.TOKEN_PRICE);
        const tokens = parseFloat(ptRow.TOKENS_PURCHASED);

        tokenPrice = isNaN(price) ? null : price;
        tokensPurchased = isNaN(tokens) ? null : tokens;
        walletAddress = ptRow.WALLET_ADDRESS || null;
    }

    // Message
    let message = null;
    if (status === "PROCESSING") {
        message = "Payment is under processing. Token allocation will be completed shortly.";
    }
    ...
    return {
        invoiceId,
        status,
        amount: status === "NOT_FOUND" ? null : amount, // clean up
        currency: status === "NOT_FOUND" ? null : currency,
        tokenPrice,
        tokensPurchased,
        walletAddress,
        timestamp,
