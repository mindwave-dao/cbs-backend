...
export async function checkPaymentStatusLogic(invoiceId, injectedSheets = null) {
    ...
    if (!sheets) {
        console.error("Database connection failed for checkPaymentStatusLogic");
        throw new Error("Database connection failed");
    }

    // STRICT: Only look in PAYMENT_TRANSACTIONS
    // Columns: INVOICE_ID (0), STATUS (1), EMAIL (2), NAME (3), EMAIL_SENT (4), EMAIL_SENT_AT (5), TOKEN_PRICE (6), TOKENS_PURCHASED (7), ADMIN_EMAIL_SENT (8), WALLET_ADDRESS (9)
    ...
    if (!ptRow) {
        return {
            status: "NOT_FOUND",
            message: "Invalid Invoice ID. No transaction found."
        };
    }

    // IF FOUND, RESOLVE STATUS
    let finalStatus = "PROCESSING"; // Default safety for known rows
    if (ptRow.STATUS === "SUCCESS") finalStatus = "SUCCESS";
    else if (ptRow.STATUS === "BLOCKED") finalStatus = "BLOCKED";
    // Any other status (PENDING, etc) stays PROCESSING for frontend display

    // DERIVE DATA
    const walletAddress = ptRow.WALLET_ADDRESS || null;
    let networkLabel = "UNKNOWN";
    if (walletAddress) {
        if (walletAddress.startsWith("0x")) networkLabel = "ETH / BSC";
        else if (walletAddress.startsWith("T")) networkLabel = "TRON";
    }

    // TIMESTAMP
    // PAYMENT_TRANSACTIONS doesn't have a creation timestamp column in standard list.
    // We can allow EMAIL_SENT_AT if available, or just null.
    // User asked for "timestamp" in response. We'll use EMAIL_SENT_AT as proxy or null.
    const timestamp = ptRow.EMAIL_SENT_AT || null;

    // RETURN DATA BASED ON STATUS
    if (finalStatus === "SUCCESS") {
        const tokenPrice = parseFloat(ptRow.TOKEN_PRICE || "0");
        const tokensPurchased = parseFloat(ptRow.TOKENS_PURCHASED || "0");
        const amountPaid = (tokenPrice > 0 && tokensPurchased > 0)
            ? (tokenPrice * tokensPurchased).toFixed(2)
            : null;
    ...
    // IF PROCESSING or BLOCKED
    return {
        invoiceId,
        status: finalStatus === "BLOCKED" ? "BLOCKED" : "PROCESSING", // "PAYMENT BLOCKED" or "PAYMENT UNDER PROCESSING" handled by UI
        amountPaid: null, // As per "Found + STATUS = PROCESSING -> Null fields" implied by "Found + STATUS = SUCCESS" details
        tokenPrice: null,
