...
export async function handlePaymentLogic(invoiceId, sourceLabel = '3THIX_API') {
    ...
    if (apiResult) {
        ...
        if (['PAID', 'COMPLETED', 'SUCCESS', 'APPROVED'].includes(rawStatus)) {
            finalStatus = 'SUCCESS';
        } else if (['UNCLAIMED', 'SUBMITTED', 'PAYMENT_SUBMITTED', 'UNPAID', 'CLAIMED', 'PROCESSING'].includes(rawStatus)) {
        ...
        // If API check specifically failed, and we are not in webhook context (where data might be passed in),
