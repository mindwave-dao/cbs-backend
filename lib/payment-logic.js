...
export async function checkPaymentStatusLogic(invoiceId, injectedSheets = null) {
    ...
    // STRICT: Only look in PAYMENT_TRANSACTIONS
    ...
    let finalStatus = "PROCESSING"; // Default safety
    if (ptRow.STATUS === "SUCCESS") finalStatus = "SUCCESS";
    else if (ptRow.STATUS === "FAILED") finalStatus = "FAILED";
    // Any other status (PENDING, etc) stays PROCESSING for frontend display
    ...
    if (finalStatus === "SUCCESS") {
        return {
            invoiceId,
            status: "SUCCESS",
            amount: ptRow.TOKEN_PRICE ? String((parseFloat(ptRow.TOKEN_PRICE) * parseFloat(ptRow.TOKENS_PURCHASED)).toFixed(2)) : null, // Approx amount or null if not stored directly in PT (PT doesn't store amount explicitly in defined columns, but we could maybe fetch it if critical. User request said "amount, token_price...". PT has token_price and tokens_purchased. Amount could be derived or we fetch from Transactions if absolutely needed? 
            // WAIT. User Request: "Status Handling: IF SUCCESS: Return amount, token_price...".
            // `PAYMENT_TRANSACTIONS` columns: INVOICE_ID, STATUS, EMAIL, NAME, EMAIL_SENT, EMAIL_SENT_AT, TOKEN_PRICE, TOKENS_PURCHASED, ADMIN_EMAIL_SENT, WALLET_ADDRESS.
            // It does NOT have AMOUNT.
            // But `findInPaymentTransactions` only returns those.
            // `Transactions` sheet HAS amount.
            // User rules said: "Sheets to READ from: 1. PAYMENT_TRANSACTIONS 2. Transactions (if needed) 3. PaymentAdditionalInfo".
            // "Search PAYMENT_TRANSACTIONS by INVOICE_ID... If found... IF STATUS === SUCCESS... Return amount".
            // So we DO need to look up Transactions OR PaymentAdditionalInfo *IF* found in PaymentTransactions, to get the amount.
            // Let's re-add lookup for metadata only if found in PT.

            // Re-think: Amount is crucial.
            // I will fetch extra data if SUCCESS.
            tokenPrice: ptRow.TOKEN_PRICE,
            tokensPurchased: ptRow.TOKENS_PURCHASED,
            walletAddress: ptRow.WALLET_ADDRESS || null,
            // Logic to get amount:
            // We can call `findInTransactions` here safely as a secondary read.
            ...await (async () => {
                const tx = await findInTransactions(sheets, invoiceId);
                if (tx) return { amount: tx.amount, currency: tx.currency, timestamp: tx.timestamp };
                return { amount: null, currency: null, timestamp: null };
            })()
        };
    }
    ...
    return {
        ...
        walletAddress: null,
        timestamp: null,
        message: "Payment is under processing. Token allocation pending."
    };
}
