...
const TRANSACTIONS_HEADERS = [
    ...
    "token_price_usd", "tokens_purchased" // New columns
];
...
async function findInPaymentTransactions(sheets, invoiceId) {
    try {
        const response = await sheets.spreadsheets.values.get({
            spreadsheetId: GOOGLE_SHEET_ID,
            range: "PAYMENT_TRANSACTIONS!A2:H" // Expanded range check (future proof)
        });
        const rows = response.data.values || [];
    ...
    try {
        ...
        if (!existing) {
            ...
            await sheets.spreadsheets.values.append({
                ...
                valueInputOption: "RAW",
                insertDataOption: "INSERT_ROWS",
                requestBody: { values: [[invoiceId, status, email || '', name || '', 'NO', '', tokenPrice, tokensPurchased]] }
            });
        }
...
export async function handlePaymentLogic(invoiceId, sourceLabel = '3THIX_API', webhookData = null) {
    ...
    if (!sheets) {
        // Fallback or error
        return { invoiceId, status: "PENDING", source: "ERROR", emailSent: false, tokens: 0, tokenPrice: 0 };
    }
    ...
    if (ptRow) {
        ...
        // --- NEW: Admin Email (Always Attempt) ---
        if (finalStatus === 'SUCCESS') {
            try {
                const { sendAdminPaymentEmail } = await import("./email.js");
    ...
    return {
        ...
        tokens: ptRow ? ptRow.TOKENS_PURCHASED : (tokensPurchased || 0),
        tokenPrice: ptRow ? ptRow.TOKEN_PRICE : (tokenPriceUsed || 0)
    };
}
