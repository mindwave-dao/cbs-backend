...
export async function submitCryptoPayment(req, res) {
    ...
    try {
        ...
        console.log(`[CRYPTO_SUBMIT] [STEP 3] Writing to CRYPTO_PAYMENTS sheet for ${email}`);
        await appendToCryptoPayments(sheetRow);
        console.log(`[CRYPTO_SUBMIT] [STEP 4] Successfully wrote to CRYPTO_PAYMENTS sheet`);
        ...
        console.log(`[CRYPTO_SUBMIT] [STEP 5] Triggering emails for ${email}...`);

        await Promise.all(emailPromises)
            .then(results => {
                const [userRes, adminRes] = results;
                console.log(`[CRYPTO_EMAILS] [COMPLETE] User Email Success: ${userRes?.success}, Admin Email Success: ${adminRes}`);
            })
            .catch(err => console.error(`[CRYPTO_EMAIL_ERROR] ${err.message}`));

        // 5. Return Success
        console.log(`[CRYPTO_SUBMIT] [SUCCESS] Transaction submitted for ${email}`);
