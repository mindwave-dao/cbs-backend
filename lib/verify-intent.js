 ...
 * Body: { intent_id: string, invoice_id?: string } or { intentId: string, invoiceId?: string }
 */
...
export async function verifyPaymentIntent(req, res) {
    ...
    try {
        // Support both snake_case (legacy/API standard) and camelCase (frontend JS standard)
        let { intent_id, invoice_id, intentId: intentIdParam, invoiceId: invoiceIdParam } = req.body;

        // Normalize to internal variable names
        // intent_id is used for 3thix API calls
        // invoiceId is used for database lookups
        intent_id = intent_id || intentIdParam;
        let invoiceId = invoice_id || invoiceIdParam;

        console.log(`[CARD_FLOW] [VERIFY] Request for invoiceId: ${invoiceId}, intent_id: ${intent_id}`);

        if (!intent_id && !invoiceId) {
        ...
        if (tx.status === 'PENDING' && intent_id) {
            ...
            // CRITICAL: We MUST await this in Vercel/Serverless or the process may exit before completion.
            try {
                const result = await finalizeSuccessfulPayment(invoiceId, { source: 'INTENT_VERIFY' });
                console.log(`[CARD_FLOW] [VERIFY] Finalization complete for ${invoiceId}:`, result.status);
            } catch (err) {
                console.error(`[CARD_FLOW] [VERIFY] Finalization error for ${invoiceId}:`, err.message);
            }
...
async function check3thixIntentStatus(intentId) {
    ...
    console.log(`[VERIFY-INTENT] 3thix Status Response:`, JSON.stringify(data));

    // Handle various success indicators
    const isSuccess = data.success || data.status === 'success' || data.remark === 'success';
