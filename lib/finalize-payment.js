...
export async function finalizeSuccessfulPayment(invoiceId, options = {}) {
    ...
    if (tokens > 0) {
        if (tx.email_sent_user !== 'YES') {
            try {
                // FIXED: Capture result and check success
                const emailResult = await sendUserPaymentSuccessEmail(email, name, invoiceId, tokens, tokenPrice, amount, walletAddress);

                if (emailResult && emailResult.success) {
                    console.log(`[PAYMENT_FINALIZATION] User email sent for ${invoiceId}`);
                    emailSent = true;
                } else {
                    console.error(`[PAYMENT_FINALIZATION] User email attempted but failed for ${invoiceId}. Error: ${emailResult?.error || 'Unknown'}`);
                }
            } catch (e) {
                console.error(`[PAYMENT_FINALIZATION] User email CRITICAL failure ${invoiceId}: ${e.message}`);
            }
        } else {
            emailSent = true;
        }
