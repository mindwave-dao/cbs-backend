...
export async function finalizeSuccessfulPayment(invoiceId, options = {}) {
    ...
    if (!email) {
        ...
        throw new Error("Missing payer email — cannot send receipt");
    }
    ...
    if (tokens > 0) {
        ...
        if (!tx || tx.email_sent_user !== 'YES') {
            try {
                ...
                // If user email fails, we should probably NOT stop the admin email, 
                // but we should definitely log it strictly.
                // However, user requirement says: "Force user email send BEFORE admin email"
            }
        } else {
