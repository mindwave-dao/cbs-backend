...
export async function handle3ThixWebhook(req, res) {
    ...
    try {
        ...
        if (normalizedStatus === 'SUCCESS') {
            console.log(`[WEBHOOK] Payment SUCCESS for ${invoiceId}`);
            ...
            finalizeSuccessfulPayment(invoiceId, { source: 'WEBHOOK' })
                .then(result => console.log(`[WEBHOOK] Finalization complete for ${invoiceId}:`, result.status))
                .catch(err => console.error(`[WEBHOOK] Finalization error for ${invoiceId}:`, err.message));
